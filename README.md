...
| AI 教练 | DeepSeek 进度诊断（优势/薄弱/风险/建议/未来 7 天）、计划调整（按周改动 + 依据，可一键采纳）。AI 问答对话已下线，见「合规说明」 |
...
| AI 问答属深度合成技术，个人主体未开放该类目 | 小程序端移除「问教练」对话（分段、消息列表、快捷问题、输入框）和 `api.chat`。云函数 `ai.chat` 保留，换企业主体后恢复前端即可 |

保留的 AI 能力是进度诊断和计划调整，两者都带「AI 生成」标识和反馈入口。

**仍有风险的地方**：按国内深度合成的界定，AI 语音合成与 AI 问答同属一类。本项目的 Azure 朗读属于语音合成，这次审核没有点名，但下一轮可能被提。进度诊断和计划调整同样是大模型生成内容，也存在被一并认定的可能。真被提出来时，可选的路径是改用企业主体，或者把对应功能也下线。

`npm test` 里的 `scripts/check-privacy-scopes.js` 会拦住前两类问题的代码回潮：需要额外声明 scope 的组件、以及重新出现在小程序端的 AI 问答入口。
