...
| 模块 | 说明 |
| --- | --- |
| 今日 | 下拉可刷新任务与公告并查看版本状态。按计划自动生成当天任务（复习 / 新单元 / 小测 / Fidel / 实战任务），记录时长、连续天数、星星和下一枚徽章 |
...
| AI 教练 | 已整体下线（微信个人主体未开放深度合成类目）。代码保留，开关见「合规说明」。原功能：DeepSeek 进度诊断、计划调整 |
...
cloudfunctions/api/     微信云函数：进度存储 + DeepSeek 诊断 / 计划调整 + Azure 语音
...
**首页下拉**也会刷新今日任务与公告，并顺带汇报版本状态：新版已下载好就弹框问要不要重启，否则用 toast 显示「已是最新版本（重开小程序可重新检查）」。

「我的 → 关于」有版本号、构建标记和「检查更新」按钮。注意微信没有「重新检查」的接口，按钮汇报的是**本次启动**的检查结果：新版已下载好就直接问要不要重启，否则显示「已是最新版本」或下载失败提示。要真正重新检查，只能退出小程序再进。

在开发者工具里验证：「编译模式」旁边的设置里勾选「下次编译时模拟更新」，重新编译就能看到弹框。

每次发版记得把 `miniprogram/config.js` 的 `buildTag` 改成这次的日期和改动摘要，它会打到 Console 并显示在关于页，用来确认用户手上跑的是哪一版。
...
| 用户输入给 AI 教练的文字（学习自述、计划诉求） | 生成进度诊断与计划调整建议；服务端只保存调用时间与字符数，不保存原文 |
...
- 服务名称：AI 学习教练（进度诊断、计划调整）与发音评分。
- 模型提供方：DeepSeek（大模型，仅用于学习诊断与计划调整）、Microsoft Azure 语音（语音合成与识别）。
...
- 使用限制：面向已登记的内部同事；每人每天 AI 调用与语音次数有上限（`cloudfunctions/api` 环境变量可调）；用户输入原文不落库。

### 合规说明（2026-09 审核驳回后的整改）

个人主体小程序提交审核时被驳回三条，整改如下。再次提审前请对照检查。

| 驳回意见 | 整改 |
| --- | --- |
| 默认自动同意隐私政策，应由用户自主选择 | 登录页的同意框默认不勾选，不勾选时登录按钮不可用，点击也只提示不登记。文案从「登录即表示同意」改为「我已阅读并同意」 |
| 一进首页未体验功能就要求授权登录 | 首页不再自动跳转登录页，昵称提示只对已登录用户显示。登录入口只在「我的」以及用户主动使用 AI / 语音时出现 |
| AI 问答属深度合成技术，个人主体未开放该类目 | 先移除「问教练」对话和 `api.chat`，随后整个「AI 教练」Tab 一并下线 |

**AI 教练现在整体关闭**。进度诊断和计划调整同样是大模型生成内容，留着有被一并认定的风险，因此不只下线问答。底部导航从 5 个 Tab 变成 4 个：今日、课程、复习、我的。课程、复习、小测、Fidel、查词句、语音朗读、跟读评分都不受影响。

开关是 `miniprogram/config.js` 的 `aiCoachEnabled`，默认 `false`。换成企业主体后恢复，要改两个地方：

1. `miniprogram/config.js` 把 `aiCoachEnabled` 改成 `true`；
2. `miniprogram/app.json` 的 `tabBar.list` 在「复习」和「我的」之间插回这一条：

```json
{ "pagePath": "pages/coach/coach", "text": "AI 教练" }
```

页面代码、云函数接口、提示词都原样保留，`ai.chat` 也在。前端的对话界面当初是删掉的，要恢复问答需要重写那部分 UI。

隐私声明页的文案跟随这个开关：关闭时不再列出 DeepSeek 和「你输入给 AI 教练的文字」。

**仍有风险的地方**：按国内深度合成的界定，AI 语音合成与 AI 问答同属一类。本项目的 Azure 朗读属于语音合成，这次审核没有点名，但下一轮仍可能被提。真被提出来时，可选的路径是改用企业主体，或者把朗读也下线（跟读评分走的是语音识别，风险低一些）。

`npm test` 里的 `scripts/check-privacy-scopes.js` 会拦住代码回潮：需要额外声明 scope 的组件、重新出现的 AI 问答入口，以及开关与 `app.json` 不一致（下线了却还留在 tabBar，或者打开了却没加回去）。
...
| `ai.chat` | `{messages, summary}` | AI 教练对话（小程序端已下线，接口保留） |
